...
public class KeyViewModelTests
{
    ...
    private static LayerColorPalette PaletteFor(Layer layer) =>
        LayerColorPalette.ForSingleLayer(layer, totalLayers: 8);

    private static KeyViewModel MakeVm(PositionedKey pk, Layer layer,
        double clusterOriginX = 0, double clusterOriginY = 0) =>
        new(pk, layer, clusterOriginX, clusterOriginY, PaletteFor(layer));

    [Fact]
    public void DisplayLabel_NormalKey_ReturnsLabel()
    {
        var vm = MakeVm(MakePosKey(label: "Space"), MakeLayer(), 0, 0);
    ...
    {
        var vm = MakeVm(MakePosKey(label: "___", transparent: true, effectiveLabel: "A"), MakeLayer(), 0, 0);
    ...
    {
        var vm = MakeVm(MakePosKey(label: "___", transparent: true), MakeLayer(), 0, 0);
    ...
    {
        var vm = MakeVm(MakePosKey(secondaryLabel: "Ctrl"), MakeLayer(), 0, 0);
    ...
    {
        var vm = MakeVm(MakePosKey(), MakeLayer(), 0, 0);
    ...
    {
        var transparent = MakeVm(MakePosKey(transparent: true), MakeLayer(), 0, 0);
        var normal = MakeVm(MakePosKey(), MakeLayer(), 0, 0);
    ...
    {
        var vm = MakeVm(MakePosKey(label: ""), MakeLayer(), 0, 0);
    ...
    {
        var vm = MakeVm(MakePosKey(label: "", transparent: true), MakeLayer(), 0, 0);
    ...
    public void Position_ScaledAt60PixelsPerUnit()
    {
        ...
        var vm = MakeVm(MakePosKey(x: 9.5, y: 2.5, width: 1.0, height: 1.0), MakeLayer(), 0, 0);
    ...
    {
        var vm = MakeVm(MakePosKey(), MakeLayer(index: 3), 0, 0);
    ...
    {
        var vm = MakeVm(MakePosKey(rawKeycode: 0x5222), MakeLayer(), 0, 0);
    ...
    {
        var vm = MakeVm(MakePosKey(transparent: true), MakeLayer(), 0, 0);
    ...
    public void Left_WithClusterOrigin_SubtractsFromBoardCoords()
    {
        ...
        var vm = MakeVm(pk, MakeLayer(), clusterOriginX: 8.5 * 60, clusterOriginY: 1.5 * 60);
    ...
    {
        var vm = MakeVm(MakePosKey(x: 7.0, y: 2.0), MakeLayer(), 0, 0);
    ...
    public void Left_RightHandClusterOrigin_SubtractsBoth()
    {
        ...
        var vm = MakeVm(pk, MakeLayer(), clusterOriginX: 15.3 * 60, clusterOriginY: 0.0);
